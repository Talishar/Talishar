  ...
  switch ($cardID) {
    ...
    case "scooba_salty_sea_dog_yellow":
      if ($from == "PLAY") {
        ...
        AddDecisionQueue("WRITELOGMZCARDLINK", $currentPlayer, "<-", 1);
    ...
    case "nimby_yellow":
    case "nimby_blue":
      MaySearchDeck($currentPlayer, "isSameName=nimblism_red", "MYHAND");
      break;
...
function SEAHitEffect($cardID): void
{
  ...
  switch ($cardID) {
    ...
    case "money_or_your_life_blue":
      ...
      AddDecisionQueue("SPECIFICCARD", $mainPlayer, "MONEYORYOURLIFE-$cardID-$repeat", 1);
      break;
...
function Tap($MZindex, $player, $tapState=1, $endStepUntap=false)
{
  ...
  if($tapState == 0 && !isUntappedPrevented($MZindex, $zoneName, $targetPlayer, $endStepUntap)) {
    if($endStepUntap && $zone[$index] == "gold_baited_hook" && GetClassState($player, piece: $CS_NumGoldCreated) <= 0 && SearchCharacterAlive($player, $zone[$index])) DestroyCharacter($player, $index);
...
function isUntappedPrevented($MZindex, $zoneName, $player, $endStepUntap=false): bool
{
  ...
  if (str_contains($zoneName, "CHAR")) {
    $uid = $zone[$index + 11] ?? "-";
    SearchCurrentTurnEffects("clap_em_in_irons_blue", $player, returnUniqueID:true) == $uid ? $untapPrevented = true : $untapPrevented = false;
  }
  elseif (str_contains($zoneName, "ALLY")) {
    $uid = $zone[$index + 5] ?? "-";
    SearchCurrentTurnEffects("clap_em_in_irons_blue", $player, returnUniqueID:true) == $uid ? $untapPrevented = true : $untapPrevented = false;
  }
