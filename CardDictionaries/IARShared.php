...
function OnlyShadowResistChoices($player, $indices) {
	foreach (explode(",", $indices) as $mzIndex) {
		$mzIndex = explode("-", $mzIndex);
		$index = intval($mzIndex[1] ?? -1);
		if ($mzIndex[0] == "MYCHAR") {
			$cardID = GetPlayerCharacter($player)[$index] ?? "-";
			if (CharacterDamagePreventionAmount($player, $index, 1, true) > 0) return false;
		}
		else if ($mzIndex[0] == "MYALLY") $cardID = GetAllies($player)[$index] ?? "-";
		else return false;
		if (WardAmount($cardID, $player) > 0 || ShadowResistAmount($cardID, $player, $index) <= 0) return false;
	}
	return true;
}
