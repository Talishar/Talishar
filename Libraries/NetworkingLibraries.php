...
function PlayCard($cardID, $from, $dynCostResolved = -1, $index = -1, $uniqueID = -1)
{
  ...
  if ($dynCostResolved == -1) {
    ...
    else WriteLog("Player " . $playerID . " " . PlayTerm($turn[0], $from) . " " . CardLink($cardID, $cardID), $turn[0] != "P" ? $currentPlayer : 0);
  ...
  if (GetClassState($currentPlayer, $CS_LastDynCost) != 0) WriteLog(CardLink($cardID, $cardID) . " was played with X = " . GetClassState($currentPlayer, $CS_LastDynCost));
  $cardType = CardType($cardID);
  $abilityType = "";
  $playType = $cardType;
