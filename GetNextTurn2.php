...
if ($lastUpdate != 0 && $cacheVal <= $lastUpdate) {
  ...
  echo (($manualMode ? "<span style='position:absolute; top:120px; left:65px;'>" . CreateButton($playerID, "-1", Mode::ManualSubHealthPoint, 0, "24px") . CreateButton($playerID, "+1", Mode::ManualAddHealthPoint, 0, "24px") . "</span>" : ""));
  ...
  echo (($manualMode ? "<span style='position:absolute; top:0px; left:65px;'>" . CreateButton($playerID, "-1", Mode::ManualSubOpponentHealthPoint, 0, "24px") . CreateButton($playerID, "+1", Mode::ManualAddOpponentHealthPoint, 0, "24px") . "</span>" : ""));
  ...
  if (CanPassPhase($turn[0]) && $currentPlayer == $playerID) echo ("<div title='Space is the shortcut to pass.' " . ProcessInputLink($playerID, Mode::Pass, 0) . " class='passButton' style='position:absolute; top:62px; left:-200px; z-index:-1; cursor:pointer; height:75px; width:225px;'>
  ...
  echo (($manualMode ? "<span style='position:absolute; top:85%; right:7%; display: inline-block;';>" . CreateButton($playerID, "-1", Mode::ManualSubActionPoint, 0, "24px") . CreateButton($playerID, "+1", Mode::ManualAddActionPoint, 0, "24px") . "</span>" : ""));
  echo ("</div></div>");
  ...
  if ($turn[0] != "OVER") {
    ...
    if ($currentPlayer == $playerID) {
      if ($turn[0] == "P" || $turn[0] == "CHOOSEHANDCANCEL" || $turn[0] == "CHOOSEDISCARDCANCEL") echo ("(" . ($turn[0] == "P" ? $myResources[0] . " of " . $myResources[1] . " " : "") . "or " . CreateButton($playerID, "Cancel", Mode::Undo, 0, "16px") . ")");
      if (CanPassPhase($turn[0])) {
        //if ($turn[0] == "B") echo (CreateButton($playerID, "Undo Block", 10001, 0, "16px"));
        if($turn[0] == "B") echo(CreateButton($playerID, "Undo Block", Mode::CancelBlock, 0, "16px") . " " . CreateButton($playerID, "Pass", Mode::Pass, 0, "16px") . " " . CreateButton($playerID, "Pass Block and Reactions", Mode::PassBlocksAndReactions, 0, "16px", "", "Reactions will not be skipped if the opponent reacts"));
      }
    }
    else {
      if($currentPlayerActivity == 2 && $playerID != 3) echo("— Opponent is inactive " . CreateButton($playerID, "Claim Victory", Mode::ClaimVictory, 0, "16px"));
    }
    echo ("</b>");
    /*
  ...
  if (IsManualMode($playerID)) echo ("&nbsp;" . CreateButton($playerID, "Turn Off Manual Mode", Mode::ChainSetting, $SET_ManualMode . "-0", "24px", "", "", true));
  echo ("</span>");
  ...
  foreach ($friendlyEffectsArr as $key => $effectArr) {
    ...
    for ($i = 0; $i < $max; ++$i) {
      ...
      $effect .= Card($key, "crops", 65, Mode::None, 1);
      $effect .= "</div>";
  ...
  if (GetClassState($playerID, $CS_NextArcaneBonus) > 0) $friendlyEffects .= "<div title='Next arcane bonus: " . GetClassState($playerID, $CS_NextArcaneBonus) . "' style='width:86px; height:66px; margin:2px; border:2px solid blue;'>" . Card("CRU161", "crops", 67, Mode::None, 0) . "</div>";
  if (GetClassState(($playerID == 1 ? 2 : 1), $CS_NextArcaneBonus) > 0) $opponentEffects .= "<div title='Next arcane bonus: " . GetClassState(($playerID == 1 ? 2 : 1), $CS_NextArcaneBonus) . "' style='width:86px; height:66px; margin:2px; border:2px solid red;'>" . Card("CRU161", "crops", 67, Mode::None, 0) . "</div>";
  ...
  if (count($chainLinks) > 0) {
    echo ("<div title='Break the Combat Chain' " . ProcessInputLink($playerID, Mode::BreakCombatChain, 0) . " class='breakChain' style='height:30px; width:60px; position:relative; display:inline-block;'></div>");
  }
  echo ("</td>");
  echo ("</tr></table>");
  if ($displayCombatChain) {
    for ($i = 0; $i < count($combatChain); $i += CombatChainPieces()) {
        $mode = (
          ...
          && IsPlayable($combatChain[$i], $turn[0], "PLAY", $i)
        ) ? Mode::PlayCombatChainAbility : Mode::None;
      $actionDisabled = 0;
      echo (Card($combatChain[$i], "concat", $cardSize, $mode, 1, $actionDisabled, $combatChain[$i + 1] == $playerID ? 1 : 2, 0, strval($i)));
    }
  }
  ...
  if (count($layers) > 0) {
    ...
    for ($i = count($layers) - LayerPieces(); $i >= 0; $i -= LayerPieces()) {
      ...
      $content .= Card($layerName, "concat", $cardSize, Mode::None, 1, 0, $layers[$i + 1] == $playerID || $playerID == 3 ? 1 : 2, controller:$layers[$i+1]);
    }
    $content .= "</div>";
  ...
  if ($turn[0] == "OVER") {
    $content = CreateButton($playerID, "Main Menu", Mode::MainMenu, 0, "24px", "", "", false, true);
    if ($playerID == 1 && $theirCharacter[0] != "DUMMY") $content .= "&nbsp;" . CreateButton($playerID, "Rematch", Mode::Rematch, 0, "24px");
    if ($playerID == 1) $content .= "&nbsp;" . CreateButton($playerID, "Quick Rematch", Mode::QuickRematch, 0, "24px");
    if ($playerID != 3) {
      $content .= "<BR><span class='Rate-Span'>Rate your Opponent:&nbsp;";
      $rating = ($playerID == 1 ? $p1PlayerRating : $p2PlayerRating);
      if ($rating != 2) {
        $content .= CreateButton($playerID, "", Mode::ThumpUp, "GreenThumb", "24px", "Images/GreenThumb.png", "👍 I liked and recommend playing with this player.");
        $content .= ($rating != 1 ? "&nbsp;" : "</span>");
      }
      if ($rating != 1) {
        $content .= CreateButton($playerID, "", Mode::ThumpDown, "RedThumb", "24px", "Images/RedThumb.png", "👎 I disliked playing with this player.") . "</span>";
      }
  ...
  if ($turn[0] == "DYNPITCH" && $turn[1] == $playerID) {
    ...
    for ($i = 0; $i < count($options); ++$i) {
      $content .= CreateButton($playerID, $options[$i], Mode::InputNumber, $options[$i], "24px");
    }
    $content .= "</div>";
  ...
  if (($turn[0] == "BUTTONINPUT" || $turn[0] == "CHOOSEARCANE" || $turn[0] == "BUTTONINPUTNOPASS") && $turn[1] == $playerID) {
    ...
    $options = explode(",", $turn[2]);
    for ($i = 0; $i < count($options); ++$i) {
      $content .= CreateButton($playerID, str_replace("_", " ", $options[$i]), Mode::ButtonInput, strval($options[$i]), "24px");
    }
    $content .= "</div>";
  ...
  if ($turn[0] == "YESNO" && $turn[1] == $playerID) {
    $content = CreateButton($playerID, "Yes", Mode::YesNo, "YES", "20px");
    $content .= CreateButton($playerID, "No", Mode::YesNo, "NO", "20px");
  ...
  {
    $content = CreateButton($playerID, "Ok", Mode::Pass, "OK", "20px");
    if (GetDQHelpText() != "-") $caption = implode(" ", explode("_", GetDQHelpText()));
    else $caption = "Choose " . TypeToPlay($turn[0]);
  ...
  if (($turn[0] == "OPT" || $turn[0] == "CHOOSETOP" || $turn[0] == "CHOOSEBOTTOM" || $turn[0] == "CHOOSECARD") && $turn[1] == $playerID) {
    ...
    for ($i = 0; $i < count($options); ++$i) {
      ...
      $content .= "</td></tr><tr><td>";
      if ($turn[0] == "CHOOSETOP" || $turn[0] == "OPT") $content .= CreateButton($playerID, "Top", Mode::ChooseTop, $options[$i], "20px");
      if ($turn[0] == "CHOOSEBOTTOM" || $turn[0] == "OPT") $content .= CreateButton($playerID, "Bottom", Mode::ChooseBottom, $options[$i], "20px");
      if ($turn[0] == "CHOOSECARD") $content .= CreateButton($playerID, "Choose", Mode::ChooseCard, $options[$i], "20px");
      $content .= "</td></tr>";
      $content .= "</table>";
      $content .= "</td>";
    }
  ...
  if ($turn[0] == "HANDTOPBOTTOM" && $turn[1] == $playerID) {
    ...
    for ($i = 0; $i < count($myHand); ++$i) {
      $content .= "<td><span>";
      $content .= CreateButton($playerID, "Top", Mode::HandToTop, $i, "20px");
      $content .= "</span><span>";
      $content .= CreateButton($playerID, "Bottom", Mode::HandToBottom, $i, "20px");
      $content .= "</span>";
      $content .= "</td>";
    }
    $content .= "</tr></table>";
  ...
  if (($turn[0] == "MAYCHOOSEMULTIZONE" || $turn[0] == "CHOOSEMULTIZONE") && $turn[1] == $playerID) {
    ...
    for ($i = 0; $i < count($options); ++$i) {
      ...
      $content .= Card($card, "concat", $cardSize, Mode::Choose, 1, 0, $playerBorderColor, $counters, $options[$i], "", false, $lifeCounters, $enduranceCounters, $atkCounters);
    }
    $content .= "</div>";
  ...
  if (($turn[0] == "MAYCHOOSEDECK" || $turn[0] == "CHOOSEDECK") && $turn[1] == $playerID) {
    ChoosePopup($myDeck, $turn[2], Mode::ChooseCardFromDeck, "Choose a card from your deck");
  }

  if ($turn[0] == "CHOOSEBANISH" && $turn[1] == $playerID) {
    ChoosePopup($myBanish, $turn[2], Mode::Choose, "Choose a card from your banish", BanishPieces());
  }

  if (($turn[0] == "MAYCHOOSEARSENAL" || $turn[0] == "CHOOSEARSENAL" || $turn[0] == "CHOOSEARSENALCANCEL") && $turn[1] == $playerID) {
    ChoosePopup($myArsenal, $turn[2], Mode::Choose, "Choose a card from your arsenal", ArsenalPieces());
  }

  if (($turn[0] == "CHOOSEPERMANENT" || $turn[0] == "MAYCHOOSEPERMANENT") && $turn[1] == $playerID) {
    $myPermanents = &GetPermanents($playerID);
    ChoosePopup($myPermanents, $turn[2], Mode::Choose, GetPhaseHelptext(), PermanentPieces());
  }

  if (($turn[0] == "CHOOSETHEIRHAND") && $turn[1] == $playerID) {
    ChoosePopup($theirHand, $turn[2], Mode::Choose, "Choose a card from your opponent hand");
  }

  if (($turn[0] == "CHOOSETHEIRAURA") && $turn[1] == $playerID) {
    ChoosePopup($theirAuras, $turn[2], Mode::Choose, "Choose one of your opponent auras");
  }
  ...
  if (($turn[0] == "CHOOSEDISCARD" || $turn[0] == "MAYCHOOSEDISCARD" || $turn[0] == "CHOOSEDISCARDCANCEL") && $turn[1] == $playerID) {
    ...
    if (GetDQHelpText() != "-") $caption = implode(" ", explode("_", GetDQHelpText()));
    ChoosePopup($myDiscard, $turn[2], Mode::Choose, $caption);
  }

  if (($turn[0] == "MAYCHOOSETHEIRDISCARD") && $turn[1] == $playerID) {
    ChoosePopup($theirDiscard, $turn[2], Mode::Choose, "Choose a card from your opponent discard");
  }

  if (($turn[0] == "CHOOSECOMBATCHAIN" || $turn[0] == "MAYCHOOSECOMBATCHAIN") && $turn[1] == $playerID) {
    ChoosePopup($combatChain, $turn[2], Mode::Choose, "Choose a card from the combat chain", CombatChainPieces());
  }

  if ($turn[0] == "CHOOSECHARACTER" && $turn[1] == $playerID) {
    ChoosePopup($myCharacter, $turn[2], Mode::Choose, "Choose a card from your character/equipment", CharacterPieces());
  }

  if ($turn[0] == "CHOOSETHEIRCHARACTER" && $turn[1] == $playerID) {
    ChoosePopup($theirCharacter, $turn[2], Mode::Choose, "Choose a card from your opponent character/equipment", CharacterPieces());
  }

  if ($turn[0] == "CHOOSETHEIRITEM" && $turn[1] == $playerID) {
    ChoosePopup($theirItems, $turn[2], Mode::Choose, "Choose one of your opponent items", ItemPieces());
  }

  if ($turn[0] == "PDECK" && $currentPlayer == $playerID) {
    $content = "";
    for ($i = 0; $i < count($myPitch); $i += 1) {
      $content .= Card($myPitch[$i], "concat", $cardSize, Mode::PitchToDeck, 1);
    }
  ...
  if (($turn[0] == "MULTICHOOSETHEIRDISCARD" || $turn[0] == "MULTICHOOSEDISCARD" || $turn[0] == "MULTICHOOSEHAND" || $turn[0] == "MAYMULTICHOOSEHAND" || $turn[0] == "MULTICHOOSEDECK" || $turn[0] == "MULTICHOOSETEXT" || $turn[0] == "MAYMULTICHOOSETEXT" || $turn[0] == "MULTICHOOSETHEIRDECK") && $currentPlayer == $playerID) {
    ...
    $content .= CreateForm($playerID, "Submit", Mode::MultiChoose, count($options));
  ...
  echo (Card($card, "concat", $cardSizeAura, Mode::None, 0, 0, 0, count($theirDiscard), controller:$otherPlayer));
  echo ("</div>");

  //Display Their Deck
  echo ("<div style='position:fixed; right:" . GetZoneRight("DECK") . "; top:" . GetZoneTop("THEIRDECK") . ";'>");
  echo (($manualMode ? "<span style='position:absolute; left:50%; -ms-transform: translateX(-50%); transform: translateX(-50%);  bottom:0px; z-index:1000;'>" . CreateButton($playerID, "Draw", Mode::ManualOpponentDrawCard, 0, "24px") . "</span>" : ""));
  $deckImage = (count($theirDeck) > 0 ? $TheirCardBack : $blankZone);
  echo (Card($deckImage, "concat", $cardSizeAura, Mode::None, 0, 0, 0, count($theirDeck)));
  echo ("</div>");
  ...
  echo (Card($card, "concat", $cardSizeAura, Mode::None, 0, 0, 0, controller:$otherPlayer));
  ...
  echo (Card($card, "concat", $cardSizeAura, Mode::None, 0, controller:$otherPlayer));
  ...
  echo (($manualMode ? "<span style='position:absolute; top:7; right:0;'>" . CreateButton($playerID, "-1", Mode::ManualOpponentSubResource, 0, "24px") . CreateButton($playerID, "+1", Mode::ManualOpponentAddResource, 0, "24px") . "</span>" : ""));
  echo ("</div>");
  ...
  if (count($landmarks) > 0) {
    ...
    for ($i = 0; $i < count($landmarks); $i += LandmarkPieces()) {
      ...
      $mode = ($playable && $currentPlayer == $playerID ? Mode::PlayLandmarkAbility : Mode::None);
      $border = CardBorderColor($landmarks[$i], "PLAY", $playable);
      $counters = 0;
      echo (Card($landmarks[$i], "concat", $cardSizeAura, $mode, 1, 0, $border, $counters, strval($i), ""));
    }
    echo ("</div>");
  }
  ...
  if (count($theirAuras) > 0) {
    for ($i = 0; $i < count($theirAuras); $i += AuraPieces()) {
      ...
      echo (Card($theirAuras[$i], "concat", $cardSizeAura, Mode::None, 1, $theirAuras[$i + 1] != 2 ? 1 : 0, 0, $counters, "", "", False, 0, 0, $atkCounters, controller:$otherPlayer) . "&nbsp");
  ...
  if (count($theirItems) > 0) {
    for ($i = 0; $i < count($theirItems); $i += ItemPieces()) {
      ...
      echo ("<div style='position:relative; display: inline-block;'>");
      echo (Card($theirItems[$i], "concat", $cardSizeAura, Mode::None, 1, $theirItems[$i + 2] != 2 ? 1 : 0, 0, $theirItems[$i + 1], "", "", false, 0, 0, 0, "ITEMS", controller:$otherPlayer) . "&nbsp");
  ...
  if (count($theirAllies) > 0) {
    for ($i = 0; $i < count($theirAllies); $i += AllyPieces()) {
      ...
      echo ("<div style='position:relative; display: inline-block;'>");
      echo (Card($theirAllies[$i], "concat", $cardSizeAura, Mode::None, 1, $theirAllies[$i + 1] != 2 ? 1 : 0, 0, 0, "", "", False, $lifeCounters, $enduranceCounters, controller:$otherPlayer) . "&nbsp");
  ...
  if (count($theirPermanents) > 0) {
    for ($i = 0; $i < count($theirPermanents); $i += PermanentPieces()) {
      ...
      echo (Card($theirPermanents[$i], "concat", $cardSizeAura, Mode::None, 1, controller:$otherPlayer) . "&nbsp");
    }
  }
  echo ("</div>");
  ...
  for ($i = 0; $i < count($theirCharacter); $i += CharacterPieces()) {
    ...
    $characterContents .= ClientRenderedCard(
      cardNumber: $theirCharacter[$i],
  ...
  if ($theirArsenal != "") {
    ...
    for ($i = 0; $i < count($theirArsenal); $i += ArsenalPieces()) {
      ...
      if($playerID == 3 && IsCasterMode()) echo (Card($theirArsenal[$i], "concat", $cardSizeAura, Mode::None, 1, $theirArsenal[$i + 2] == 0 ? 1 : 0, 0, $theirArsenal[$i + 3], controller:$otherPlayer));
      else if ($theirArsenal[$i + 1] == "UP") echo (Card($theirArsenal[$i], "concat", $cardSizeAura, Mode::None, 1, $theirArsenal[$i + 2] == 0 ? 1 : 0, 0, $theirArsenal[$i + 3], controller:$otherPlayer));
      else echo (Card($TheirCardBack, "concat", $cardSizeAura, Mode::None, 0));
  ...
  if (strpos($turn[0], "CHOOSEHAND") !== false && ($turn[0] != "MULTICHOOSEHAND" || $turn[0] != "MAYMULTICHOOSEHAND")) $mode = Mode::Choose;
  ...
  for ($i = 0; $i < count($myHand); ++$i) {
    ...
    if ($playerID == 3) {
      ...
      if ($playerID == $currentPlayer) $playable = $turn[0] == "ARS" || IsPlayable($myHand[$i], $turn[0], "HAND", -1, $restriction) || ($mode == Mode::Choose && strpos("," . $turn[2] . ",", "," . $i . ",") !== false);
      else $playable = false;
  ...
  if (count($myArsenal) > 0) {
    $arsenalLeft = (
        count($myArsenal) == ArsenalPieces()
            ? "calc(50% - " . (intval($cardWidth / 2) + 6) . "px)"
            : "calc(50% - " . (intval($cardWidth) + 14) . "px)");
    ...
    for ($i = 0; $i < count($myArsenal); $i += ArsenalPieces()) {
      echo ("<div style='position:relative; display:inline-block'>");
      if ($playerID == 3) {
        if ($myArsenal[$i + 1] == "UP" || IsCasterMode()) {
            echo(Card($myArsenal[$i], "concat", $cardSizeAura, Mode::None, 1, $myArsenal[$i + 2] == 0 ? 1 : 0, 0, $myArsenal[$i + 3]));
        } else {
            echo(Card($MyCardBack, "concat", $cardSizeAura, Mode::None, 0, 0, 0));
        }
      } else {
        ...
        echo (Card($myArsenal[$i], "concat", $cardSizeAura, $currentPlayer == $playerID && $playable ? Mode::PlayFromArsenal : Mode::None, 1, $myArsenal[$i + 2] > 0 ? 0 : 1, $border, $counters, strval($i), controller:$playerID));
  ...
  if (count($myAuras) > 0) {
    for ($i = 0; $i < count($myAuras); $i += AuraPieces()) {
      ...
      echo ("<div style='position:relative; display: inline-block;'>");
      echo (Card($myAuras[$i], "concat", $cardSizeAura, $currentPlayer == $playerID && $turn[0] != "P" && $playable ? Mode::PlayAuraAbility : Mode::None, 1, $myAuras[$i + 1] != 2 ? 1 : 0, $border, $counters, strval($i), "", False, 0, 0, $atkCounters, controller:$playerID) . "&nbsp");
  ...
  if (count($myItems) > 0) {
    for ($i = 0; $i < count($myItems); $i += ItemPieces()) {
      ...
      echo ("<div style='position:relative; display: inline-block;'>");
      echo (Card($myItems[$i], "concat", $cardSizeAura, $currentPlayer == $playerID && $turn[0] != "P" && $playable ? Mode::PlayItemAbility : Mode::None, 1, $myItems[$i + 2] != 2 ? 1 : 0, $border, $myItems[$i + 1], strval($i), "", false, 0, 0, 0, "ITEMS", controller:$playerID) . "&nbsp");
  ...
  if (count($myAllies) > 0) {
    for ($i = 0; $i < count($myAllies); $i += AllyPieces()) {
      ...
      echo ("<div style='position:relative; display: inline-block;'>");
      echo (Card($myAllies[$i], "concat", $cardSizeAura, $currentPlayer == $playerID && $turn[0] != "P" && $playable ? Mode::PlayAllyAbility : Mode::None, 1, $myAllies[$i + 1] != 2 ? 1 : 0, $border, 0, strval($i), "", False, $lifeCounters, $enduranceCounters, controller:$playerID) . "&nbsp");
  ...
  if (count($myPermanents) > 0) {
    for ($i = 0; $i < count($myPermanents); $i += PermanentPieces()) {
      ...
      echo (Card($myPermanents[$i], "concat", $cardSizeAura, Mode::None, 1, controller:$playerID) . "&nbsp");
    }
  }
  echo ("</div>");
  ...
  for ($i = 0; $i < count($myCharacter); $i += CharacterPieces()) {
    ...
    $myCharData .= ClientRenderedCard($myCharacter[$i], $currentPlayer == $playerID && $playable ? Mode::PlayEquipmentAbility : Mode::None, $myCharacter[$i + 1] != 2 ? 1 : 0, $border, $myCharacter[$i + 1] != 0 ? $counters : 0, strval($i), 0, $myCharacter[$i + 4], $atkCounters, $playerID, $type, $sType, $restriction, $myCharacter[$i + 1] == 0, $myCharacter[$i + 6] == 1, $myCharacter[$i + 8] == 1, $gem);
  }
  ...
  echo (Card($card, "concat", $cardSizeAura, Mode::None, 0, 0, 0, count($myDiscard), controller:$playerID));
  echo ("</div>");
  ...
  echo (($manualMode ? "<span style='position:absolute; left:50%; -ms-transform: translateX(-50%); transform: translateX(-50%); bottom:0px; z-index:1000;'>" . CreateButton($playerID, "Draw", Mode::ManualDrawCard, 0, "24px") . "</span>" : ""));
  $deckImage = (count($myDeck) > 0 ? $MyCardBack : $blankZone);
  echo (Card($deckImage, "concat", $cardSizeAura, Mode::None, 0, 0, 0, count($myDeck)));
  echo ("</div>");
  ...
  echo (Card($card, "concat", $cardSizeAura, Mode::None, 0, 0, 0, controller:$playerID));
  ...
  echo (Card($card, "concat", $cardSizeAura, Mode::None, 0, controller:$playerID));
  ...
  echo (($manualMode ? "<span style='position:absolute; padding-bottom: 5px; bottom:0; right:0;'>" . CreateButton($playerID, "-1", Mode::ManualSubResource, 0, "24px") . CreateButton($playerID, "+1", Mode::ManualAddResource, 0, "24px") . "</span>" : ""));
  echo ("</div>");
...
function DisplayTiles($player)
{
  ...
  if ($count > 0) {
    echo ("<div style='position:relative; display: inline-block;'>");
    echo (Card("WTR075", "concat", $cardSizeAura, Mode::None, 1, 0, 0, ($count > 1 ? $count : 0)) . "&nbsp");
  ...
  if ($count > 0) {
    echo ("<div style='position:relative; display: inline-block;'>");
    echo (Card("ARC112", "concat", $cardSizeAura, Mode::None, 1, 0, 0, ($count > 1 ? $count : 0)) . "&nbsp");
    DisplayPriorityGem(($player == $playerID ? $auras[$first + 7] : $auras[$first + 8]), "AURAS-" . $first, ($player != $playerID ? 1 : 0));
    echo ("</div>");
  }
  ...
  if ($soulShackleCount > 0) echo (Card("MON186", "concat", $cardSizeAura, Mode::None, 1, 0, 0, ($soulShackleCount > 1 ? $soulShackleCount : 0)) . "&nbsp");
  ...
  if ($frostbiteCount > 0) {
    echo ("<div style='position:relative; display: inline-block;'>");
    echo (Card("ELE111", "concat", $cardSizeAura, Mode::None, 1, 0, 0, ($frostbiteCount > 1 ? $frostbiteCount : 0)) . "&nbsp");
    DisplayPriorityGem(($player == $playerID ? $auras[$first + 7] : $auras[$first + 8]), "AURAS-" . $first, ($player != $playerID ? 1 : 0));
    echo ("</div>");
  }
  ...
  if ($copperCount > 0) echo (Card("CRU197", "concat", $cardSizeAura, Mode::None, 1, 0, 0, ($copperCount > 1 ? $copperCount : 0)) . "&nbsp");
  ...
  if ($ashCount > 0) echo (Card("UPR043", "concat", $cardSizeAura, Mode::None, 1, 0, 0, ($ashCount > 1 ? $ashCount : 0)) . "&nbsp");
}
...
function DisplayPriorityGem($setting, $MZindex, $otherPlayer = 0)
{
  ...
  if ($setting != 2 && $playerID != 3) {
    ...
    if ($setting == 0) echo ("<img " . ProcessInputLink($playerID, ($otherPlayer ? Mode::ToggleOpponentPermanent : Mode::TogglePermanent), $MZindex) . " title='Not holding priority' style='position:absolute; display: inline-block; z-index:1001; " . $position . " left:" . $cardWidth / 2 - 13 . "px; width:40px; height:40px; cursor:pointer;' src='./Images/$gem' />");
    else if ($setting == 1) echo ("<img " . ProcessInputLink($playerID, ($otherPlayer ? Mode::ToggleOpponentPermanent : Mode::TogglePermanent), $MZindex) . " title='Holding priority' style='position:absolute; display: inline-block; z-index:1001; " . $position . " left:" . $cardWidth / 2 - 13 . "px; width:40px; height:40px; cursor:pointer;' src='./Images/$gem' />");
  }
}
