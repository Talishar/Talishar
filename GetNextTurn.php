...
if ($lastUpdate != 0 && $cacheVal <= $lastUpdate) {
  ...
  <span style='position:absolute; left:100px; top:15px; color: " . $fontColor . "; font-family:Helvetica; font-size:36px; font-weight: 550; text-shadow: 2px 0 0 " . $borderColor . ", 0 -2px 0 " . $borderColor . ", 0 2px 0 " . $borderColor . ", -2px 0 0 " . $borderColor . "; user-select: none;'>Pass</span>
  <span style='position:absolute; bottom:7px; left:64%; -ms-transform: translateX(-50%); transform: translateX(-50%); color: " . $fontColor . "; font-family:Helvetica; font-size:14px; text-shadow: 2px 0 0 " . $borderColor . ", 0 -2px 0 " . $borderColor . ", 0 2px 0 " . $borderColor . ", -2px 0 0 " . $borderColor . ";'>[Space]</span></div>");
  else {
    echo ("<div title='Space is the shortcut to pass.' class='passInactive' style='position:absolute; top:62px; left:-200px; z-index:-1; height:75px; width:225px;'><span style='position:absolute; left:100px; top:15px; color:gray; font-family:Helvetica; font-size:36px; font-weight: 550; text-shadow: 1px 0 0 " . $borderColor . ", 0 -1px 0 " . $borderColor . ", 0 1px 0 " . $borderColor . ", -1px 0 0 " . $borderColor . "; user-select: none;'>Pass</span>");
    echo ("<span style='position:absolute; bottom:7px; left:64%; -ms-transform: translateX(-50%); transform: translateX(-50%); color:gray; font-family:Helvetica; font-size:14px; text-shadow: 2px 0 0 " . $borderColor . ", 0 -2px 0 " . $borderColor . ", 0 2px 0 " . $borderColor . ", -2px 0 0 " . $borderColor . ";'>[Space]</span></div>");
  }
  echo ("<div style='position:absolute; top:117px; left:-150px; z-index:-4;'><img style='height:60px; width:170px;' src='./Images/p1APTracker.png' /><span style='position:absolute; left:85; top:20; z-index:10; color:#111; font-weight:600; font-size:30px;'>" . $actionPoints . "AP" . "</span>");
  ...
  if ($displayCombatChain) {
    ...
    // TODO: Uncomment when boost is an additional cost.
    //if($combatChainState[$CCS_RequiredEquipmentBlock] > 0) echo("<td style='padding-left:5px; font-size:18px; font-weight:650; color: ".$fontColor."; text-shadow: 2px 0 0 ".$borderColor.", 0 -2px 0 ".$borderColor.", 0 2px 0 ".$borderColor.", -2px 0 0 ".$borderColor."';>Block With Equipment Required</td>");
  ...
  for ($i = 0; $i < count($theirCharacter); $i += CharacterPieces()) {
    ...
    echo (Card($theirCharacter[$i], "concat", $cardSizeEquipment, 0, 1, $theirCharacter[$i + 1] != 2 ? 1 : 0, 0, $theirCharacter[$i + 1] != 0 ? $counters : 0, "", "", false, 0, $theirCharacter[$i + 4], $atkCounters, "CHARACTER"));
  ...
  if (count($myArsenal) > 0) {
    ...
    for ($i = 0; $i < count($myArsenal); $i += ArsenalPieces()) {
      ...
      if ($playerID == 3) {
        ...
        echo (Card($myArsenal[$i], "concat", $cardSizeAura, $currentPlayer == $playerID && $playable ? 5 : 0, 1, $myArsenal[$i + 2] > 0 ? 0 : 1, $border, $counters, strval($i)));
  ...
  for ($i = 0; $i < count($myCharacter); $i += CharacterPieces()) {
    ...
    echo (Card($myCharacter[$i], "concat", $cardSizeEquipment, $currentPlayer == $playerID && $playable ? 3 : 0, 1, $myCharacter[$i + 1] != 2 ? 1 : 0, $border, $myCharacter[$i + 1] != 0 ? $counters : 0, strval($i), "", false, 0, $myCharacter[$i + 4], $atkCounters, "CHARACTER"));
    if ($restriction != "") {
  ...
  echo ("<div style='text-align:center; margin-top: 3px; width:100%; font-weight:bold; font-size:24; font-weight: 550; color: " . $fontColor . "; text-shadow: 2px 0 0 " . $borderColor . ", 0 -2px 0 " . $borderColor . ", 0 2px 0 " . $borderColor . ", -2px 0 0 " . $borderColor . ";'>Turn #" . $currentTurn . "</div>");
  echo ("<div style='text-align:center; width:100%; font-weight:bold; font-size:16; font-weight: 550; color: " . $fontColor . "; text-shadow: 2px 0 0 " . $borderColor . ", 0 -2px 0 " . $borderColor . ", 0 2px 0 " . $borderColor . ", -2px 0 0 " . $borderColor . ";'>Last Played</div>");
  ...
  echo ("</div>");
  echo ("<div style='position:relative; z-index:-1; left:3px; top:-8px;'><img style='height:100px; width:100%;' src='./Images/phaseTracker2.png' />");
  ...
  echo ("<div id='gamelog' style='flex-grow:1; border: 3px solid " . $borderColor . "; border-radius: 6px; position:relative; background-color: " . $backgroundColor . "; overflow-y: auto;'>");
  EchoLog($gameName, $playerID);
  echo ("</div>");
  echo("<div id='chatPlaceholder' style='height:26px;'></div>");
  echo ("</div>");
