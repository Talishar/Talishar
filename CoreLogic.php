...
function EvaluateCombatChain(&$totalPower, &$totalDefense, &$powerModifiers = [], $secondNeedleCheck = false)
{
  ...
  if(isset($combatChain[10]) && $combatChain[10] !== "-") {
    ...
    foreach ($buffs as $buffSetID) {
      ...
      if (($canGainAttack || $power < 0) && !($snagActive && ($buff == $attackID || CardType(EffectCardID($buff)) == "AR"))) {
        $powerModifiers[] = $buff;
        $powerModifiers[] = $power;
        AddPower($totalPower, $power);
      }
...
function AddCombatChain($cardID, $player, $from, $resourcesPaid, $OriginUniqueID, $defending=false)
{
  ...
  $combatChain[] = ($turn[0] != "B" && !$defending) ? GetClassState($player, $CS_ResolvingLayerUniqueID) : GetUniqueId($cardID, $player);
...
function DealDamageAsync($player, $damage, $type, $source, $playerSource)
{
  ...
  $damage = CombatChainTakeDamageAbilities($player, $damage, $type, $source);
  $damage = StackTakeDamageAbilities($player, $damage, $type, $source);
...
function AddDamagePreventionSelection($player, $damage, $type, $preventable, $source, $shadowResist = false)
{
  ...
  PrependDecisionQueue($shadowResist ? "CHOOSEDAMAGEPREVENTION" : "CHOOSEMULTIZONE", $player, "<-", 1);
...
function CurrentEffectDamageEffects($target, $source, $type, $damage, $playerSource)
{
  ...
  if ($damageContextUID == $AttackCard->UniqueID()) {
    $effects = explode(",",$AttackCard->StaticBuffs());
...
function GetChainLinkCards($playerID = "", $cardType = "", $exclCardTypes = "", $nameContains = "", $subType = "", $exclCardSubTypes = "", $asMZInd = false, $color = "")
{
  global $combatChain;
  $piecesArr = [];
  $exclCardTypes = explode(",", $exclCardTypes) ?? [];
  // $exclTypeSet = $exclCardTypes !== "" ? array_flip() : [];
  ...
  for ($i = 0; $i < $combatChainCount; $i += $combatChainPieces) {
    ...
    foreach ($exclCardTypes as $exclCardType) {
      if (TypeContains($combatChain[$i], $exclCardType)) continue 2;
    }
    $excluded = false;
...
function ResolutionStepEffectTriggers()
{
  ...
  for ($i = count($currentTurnEffects) - $currentTurnEffectsPieces; $i >= 0; $i -= $currentTurnEffectsPieces) {
    ...
    if (!isset($classExistsCache[$effectClass])) {
      $classExistsCache[$effectClass] = class_exists($effectClass);
    }
    if ($classExistsCache[$effectClass]) {
...
function PlayAbility($cardID, $from, $resourcesPaid, $target = "-", $additionalCosts = "-")
{
  ...
  if (HasMeld($cardID)) {
    ...
    AddDecisionQueue("APPENDLASTRESULT", $currentPlayer, "|$target|$from", 1);
  ...
  else if ($set == "ARR") return ARRPlayAbility($cardID, $from, $resourcesPaid, $target, $additionalCosts);
  else if ($set == "AAC") return AACPlayAbility($cardID, $from, $resourcesPaid, $target, $additionalCosts);
  else if ($set == "PEN") return PENPlayAbility($cardID, $from, $resourcesPaid, $target, $additionalCosts);
  else {
    switch ($cardID) {
...
function Draw($player, $mainPhase = true, $fromCardEffect = true, $effectSource = "-", $num=1)
{
  ...
  if ($mainPhase && (SearchCharacterActive($otherPlayer, "valda_brightaxe") || SearchCurrentTurnEffects("valda_brightaxe-SHIYANA", $otherPlayer) && SearchCharacterActive($otherPlayer, "shiyana_diamond_gemini"))) {
    QueueValdaSeismicSurge($otherPlayer, "valda_brightaxe", $num);
  }
  if ($mainPhase && (SearchCharacterActive($otherPlayer, "valda_seismic_impact") || SearchCurrentTurnEffects("valda_seismic_impact-SHIYANA", $otherPlayer) && SearchCharacterActive($otherPlayer, "shiyana_diamond_gemini"))) {
    QueueValdaSeismicSurge($otherPlayer, "valda_seismic_impact", $num);
  }
...
function ResolveGoesWhere($goesWhere, $cardID, $player, $from, $effectController = "", $modifier = "NA", $additionalCosts = "-")
{
  ...
  switch ($goesWhere) {
    ...
    case "THEIRBANISH":
      ...
      break;
    case "THEIRDISCARD":
      return AddGraveyard($cardID, $otherPlayer, $from, $effectController, $player);
    case "THEIRBOTDECK":
