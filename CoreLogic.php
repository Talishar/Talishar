...
function CanPlayAsInstant($cardID, $index=-1, $from="")
{
  ...
  else if($cardID == "DTD088" || $cardID == "DTD089" || $cardID == "DTD090") {return GetClassState($currentPlayer, $CS_NumAddedToSoul); }
...
function IsAlternativeCostPaid($cardID, $from)
{
  ...
  for($i = count($currentTurnEffects) - CurrentTurnPieces(); $i >= 0; $i -= CurrentTurnPieces()) {
    ...
    if($currentTurnEffects[$i + 1] == $currentPlayer) {
      switch($currentTurnEffects[$i]) {
        case "ARC185": case "CRU188": case "MON199": case "MON257": case "EVR161": case "HVY176-PAID": case "MST131":
...
function PlayAbility($cardID, $from, $resourcesPaid, $target = "-", $additionalCosts = "-")
{
  ...
  else if($set == "AUR" || $set == "TER") return FSPlayAbility($cardID, $from, $resourcesPaid, $target, $additionalCosts);
  else { 
    switch ($cardID) {
