...
function EvaluateCombatChain(&$totalPower, &$totalDefense, &$powerModifiers = [], $secondNeedleCheck = false)
{
  ...
  if ($combatChainState[$CCS_WeaponIndex] != -1) {
    ...
    if (DelimStringContains($attackType, "W")) {
      if (SubtypeContains($CombatChain->AttackCard()->ID(), "Item")) {
...
function ResolveCard($cardID, $from, $definedCardType, $definedCardSubType, $index, $uniqueID, $isBlock, $additionalCosts, $target, $resourcesPaid) {
  global $currentPlayer, $combatChain, $layers, $mainPlayer, $combatChainState, $CCS_AttackTarget, $CombatChain, $currentTurnEffects, $CS_PlayCCIndex;
  global $defPlayer, $SET_PassDRStep, $CCS_AttackPlayedFrom, $CCS_AttackUniqueID, $CS_NumLess3PowAAPlayed, $CS_PlayIndex, $CCS_WeaponIndex, $turn;
  global $CCS_AttackFused, $CS_NumTimesAttacked, $CS_NumDragonAttacks, $CS_NumIllusionistAttacks, $CS_NumIllusionistActionCardAttacks, $CS_NumBlueDefended;
  if(GoesOnCombatChain($turn[0], $cardID, $from, $currentPlayer)) {
    ...
    if ($target == "MISSINGTARGET") { //if only spectra was targeted
      $goesWhere = GoesWhereAfterResolving($cardID, $from, $currentPlayer, additionalCosts: $additionalCosts);
      if(CardType($cardID) != "T" && CardType($cardID) != "Macro" && !IsActivated($cardID, $from)) //Don't need to add to anywhere if it's a token
    ...
    if ($definedCardType != "C" && $definedCardType != "E" && $definedCardType != "W" && $definedCardType != "Macro") {
      $goesWhere = GoesWhereAfterResolving($cardID, $from, $currentPlayer, additionalCosts: $additionalCosts);
