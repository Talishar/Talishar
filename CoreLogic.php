...
function DoesAttackHaveGoAgain()
{
  ...
  if(ClassContains($attackID, "ILLUSIONIST", $mainPlayer)) {
    ...
    if($isAura && SearchCharacterForCard($mainPlayer, "MST130") && $auras[$combatChainState[$CCS_WeaponIndex]+3] > 0) return true;
  }
...
function CloseCombatChain($chainClosed="true")
{
  ...
  if(count($layers) <= LayerPieces() && IsGamePhase($layers[0])) $layers = [];//In case there's another combat chain related layer like defense step
...
{
  global $combatChainState, $CCS_ChainLinkHitEffectsPrevented, $mainPlayer, $layers;
