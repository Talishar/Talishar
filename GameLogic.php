...
function DecisionQueueStaticEffect($phase, $player, $parameter, $lastResult)
{
  ...
  switch ($phase) {
      ...
      switch ($parameter) {
        ...
        case "no_fear_red":
          ...
          for ($i = 0; $i < $handCount; $i += $handPieces) {
            if (CardType($hand[$i]) == "AA" && ModifiedPowerValue($hand[$i], $player, "HAND", "no_fear_red") >= 6) $rv[] = $i;
          }
          $rv = implode(",", $rv);
        ...
        case "chart_the_high_seas_blue_1":
          ...
          $rv = [];
          for ($i = 0; $i < $amount; ++$i) {
            if (ColorContains($topX[$i], 3, $player)) $rv[] = "MYDECK-$i";
          }
          $rv = implode(",", $rv);
          break;
        case "PRELAYERS":
          ...
          for ($i = 0; $i < $preLayersCount; $i += $layerPieces) {
            if ($preLayers[$i + 1] == $player) $rv[] = "PRELAYERS-$i";
          }
          $rv = implode(",", $rv);
          break;
        case "PRELAYERIDS":
        ...
        case "OPPSENERGYPERMANENTS":
          ...
          for ($i = 0; $i < $characterCount; $i += $characterPieces) {
            if (HasEnergyCounters($character, $i)) $rv[] = $prefix."CHAR-$i";
          }
          $auras = GetAuras($playerID);
          $aurasCount = count($auras);
          $auraPieces = AuraPieces();
          for ($i = 0; $i < $aurasCount; $i += $auraPieces) {
            if (HasEnergyCounters($auras, $i)) $rv[] = $prefix."AURAS-$i";
          }
          $rv = implode(",", $rv);
          break;
        default:
    ...
    case "SCOURINDICES":
      ...
      $currentTargets = array_flip(explode(",", $scourParts[1]));
      ...
      foreach ($rvOrig as $ind) {
        if (!isset($currentTargets[$ind])) $rv[] = $ind;
      }
      $rv = implode(",", $rv);
      return $rv == "" ? "PASS" : $rv;
    case "MULTITARGETINDICES":
      $currentTargets = array_flip(explode(",", $lastResult));
      $rvOrig = explode(",", SearchMultizone($player, $parameter));
      $rv = [];
      //remove any choices that have already been targeted
      foreach ($rvOrig as $ind) {
        if (!isset($currentTargets[CleanTarget($player, $ind)])) $rv[] = $ind;
      }
      $rv = implode(",", $rv);
      return $rv == "" ? "PASS" : $rv;
    case "ALLYINDICES":
      $currentTargets = array_flip(explode(",", $parameter));
    ...
    case "BLOCKLESS0HAND":
      ...
      $searchResult = SearchMultiZoneFormat(implode(",", $cardListArr), "THEIRHAND");
    ...
    case "MULTIBANISH":
      ...
      for ($i = 0; $i < $cardsCount; ++$i) {
        $mzIndicesArr[] = "BANISH-" . BanishCardForPlayer($cards[$i], $player, $params[0], $params[1] ?? "-", $params[2] ?? "");
      }
      $dqState[5] = implode(",", $mzIndicesArr);
      return $lastResult;
    ...
    case "COMBATCHAINPOWERMODIFIER":
      ...
      if ($skip == "-") {
        if ($val > 0) WriteLog(CardLink($cardID) . " gets +" . $val . " power");
        else if ($val < 0) WriteLog(CardLink($cardID) . " loses " . -$val . " power");
      }
      return $lastResult;
    ...
    case "PUTINANYORDER":
      ...
      for ($i = 0; $i < $parameter; ++$i) {
        if ($deck->RemainingCards() > 0) {
          $reorderCardsArr[] = $deck->Top(remove: true);
        }
      }
      if ($reorderCardsArr !== []) {
        PrependDecisionQueue("CHOOSETOPOPPONENT", $player, implode(",", $reorderCardsArr));
    ...
    case "MULTIREMOVEDISCARD":
      ...
      for ($i = 0; $i < $lastResultCount; ++$i) {
        ...
        if ($parameter == "1") WriteLog(CardLink($discard[$lastResult[$i]]));
    ...
    case "ADDMYPITCH":
      ...
      WriteLog("Player " . $player . " pitched " . CardLink($lastResult));
      $pitch[] = $lastResult;
      $pitch[] = GetUniqueId($lastResult, $player);
      return $lastResult;
    case "PITCHABILITY":
    ...
    case "DESTROYEQUIPDEF0":
      ...
      if ($baseBlock + $character[$lastResult + 4] + BlockModifier($cardID, "EQUIP", 0, $lastResult)<= 0 && $baseBlock != -1) {
        WriteLog(CardLink($character[$lastResult]) . " was destroyed");
        ...
        case "THEIRCHAR":
          if ($character[$lrArr[1] + 5] < 2 && SearchCurrentTurnEffects("blood_on_her_hands_yellow", $player, returnUniqueID: true) != $character[$lrArr[1] + 11]) {
            ...
            if ($otherFlurry->Index() == -1) {
              ...
              WriteLog(CardLink($character[$lrArr[1]]) . " has already been flurried!");
            }
            break;
          }
        default:
    ...
    case "OP":
      ...
      switch ($params[0]) {
        ...
        case "REMOVECARD":
          ...
          $idx = array_search($lastResult, $cards, true);
          if ($idx !== false) {
    ...
    case "MULTIADDDECK":
      if ($lastResult != "") {
        $deck = new Deck($player);
        foreach (explode(",", $lastResult) as $card) {
          if (CardName($card) != "") $deck->AddBottom($card);
    ...
    case "MULTIADDTOPDECK":
      $deck = new Deck($player);
      foreach (explode(",", $lastResult) as $card) {
        if ($parameter == "1") WriteLog(CardLink($card));
        $deck->AddTop($card);
      }
      return $lastResult;
    ...
    case "DESTROYTOPCARD":
      $deck = new Deck($player);
      WriteLog("Destroyed " . CardLink($deck->Top()) . " on top of Player " . $player . " deck");
    ...
    case "SHOWMODES":
      ...
      for ($i = 0; $i < $modesCount; ++$i) {
        if ($text != "") $text .= ", ";
        if ($i > 0 && $i == $modesCount - 1) $text .= " and ";
        $text .= GamestateUnsanitize($modes[$i]);
      }
      if ($text == "") $text = "None";
      WriteLog("Selected mode" . ($modesCount > 1 ? "s" : "") . " for " . CardLink($parameter) . ($modesCount > 1 ? " are" : " is") . ": " . $text);
      return $lastResult;
    case "REVEALCARDS":
    ...
    case "REVEALHANDCARDS":
      ...
      for ($i = 0; $i < $indicesCount; ++$i) {
        if (isset($hand[$indices[$i]])) {
          $cardsArr[] = $hand[$indices[$i]];
        }
      }
      $cards = implode(",", $cardsArr);
    ...
    case "SHOWHANDWRITELOG":
      ...
      for ($i = 0; $i < $handCount; ++$i) {
        $cardsArr[] = CardLink($hand[$i]);
      }
      if ($cardsArr !== []) WriteLog("Hand content: " . implode(", ", $cardsArr) . ".");
      return $lastResult;
    case "SHOWARSENALWRITELOG":
      ...
      for ($i = 0; $i < $arsenalCount; $i += $arsenalPieces) {
        $cardsArr[] = CardLink($arsenal[$i]);
      }
      if ($cardsArr !== []) WriteLog("Arsenal contents: " . implode(", ", $cardsArr) . ".");
      return $lastResult;
    ...
    case "WRITELOGCARDLINK":
      WriteLog(CardLink($parameter) . " was chosen");
      return $lastResult;
    ...
    case "ADDSTASISTURNEFFECT":
      $character = &GetPlayerCharacter($player);
      WriteLog(CardLink($character[$lastResult]) . " can't be activated this turn.");
    ...
    case "EQUIPCANTDEFEND":
      $character = &GetPlayerCharacter($player);
      WriteLog(CardLink($character[$lastResult]) . " can't defend this turn.");
      $effect = $parameter . $character[$lastResult];
      AddCurrentTurnEffect($effect, $player);
      return $lastResult;
    ...
    case "NOTEQUALNAMEPASS":
      $name1 = GamestateUnsanitize($lastResult);
      $name2 = GamestateUnsanitize($parameter);
    ...
    case "ALLCARDTYPEORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!TypeContains($card, $parameter, $player)) return "PASS";
      }
      return $lastResult;
    case "ALLCARDWATERYGRAVEORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!HasWateryGrave($card)) return "PASS";
      }
      return $lastResult;
    ...
    case "NONECARDPITCHORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (PitchValue($card) == $parameter) return "PASS";
      }
      return $lastResult;
    case "ALLCARDSUBTYPEORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!SubtypeContains($card, $parameter, $player)) return "PASS";
      }
      return $lastResult;
    case "ALLCARDTALENTORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!TalentContains($card, $parameter, $player)) return "PASS";
      }
      return $lastResult;
    ...
    case "ALLCARDCOLORORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!ColorContains($card, $parameter, $player)) return "PASS";
      }
      return $lastResult;
    case "ALLCARDSCOMBOORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!HasCombo($card)) return "PASS";
      }
      return $lastResult;
    case "ALLCARDMAXCOSTORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (CardCost($card) > $parameter) return "PASS";
      }
      return $lastResult;
    case "ALLCARDCLASSORPASS":
      foreach (explode(",", $lastResult) as $card) {
        if (!ClassContains($card, $parameter, $player)) return "PASS";
      }
      return $lastResult;
    ...
    case "TRIPWIRETRAP":
      if ($lastResult == 0) {
        WriteLog("Hit effects are prevented by " . CardLink("tripwire_trap_red") . " this chain link");
    ...
    case "FINISHCHARGE":
      ...
      WriteLog(CardLink($lastResult) . " was charged.");
    ...
    case "TAKEDAMAGE":
      ...
      WriteLog("Player $player is about to take $damage damage from " . Cardlink($source));
      return $damage;
    case "AFTERQUELL":
    ...
    case "DEALARCANE":
      if ($lastResult != "-") { // make sure the target is still there
        ...
        if ($type == "PLAYCARD") {
          ...
          WriteLog(CardLink($sourceID) . " is dealing " . $damage . " arcane damage");
        }
        if ($type == "ARCANESHOCK") {
    ...
    case "TAKEARCANE":
      ...
      if ($damage < 0) $damage = 0;
      WriteLog("Player " . $playerSource . " is dealing $damage arcane damage from " . CardLink($source), $player);
    ...
    case "INVERTEXISTENCE":
      if ($lastResult == "") {
        WriteLog("No cards were selected, " . CardLink("invert_existence_blue") . " did not banish any cards");
        return $lastResult;
      }
      $cards = explode(",", $lastResult);
      $numAA = 0;
      $numNAA = 0;
      $message = CardLink("invert_existence_blue") . " banished ";
      $cardsCount = count($cards);
      for ($i = 0; $i < $cardsCount; ++$i) {
        ...
        if ($i >= 1) $message .= ", ";
        if ($i != 0 && $i == $cardsCount - 1) $message .= "and ";
        $message .= CardLink($cards[$i]);
      }
      WriteLog($message);
    ...
    case "ROUSETHEANCIENTS":
      ...
      foreach ($cards as $card) {
        $totalAV += intval(ModifiedPowerValue($card, $player, "HAND", source: "rouse_the_ancients_blue"));
      }
      if ($totalAV >= 13) {
        AddCurrentTurnEffect("rouse_the_ancients_blue", $player);
        WriteLog(CardLink("rouse_the_ancients_blue") . " got +7 and go again");
      }
      return $lastResult;
    case "TOOTHANDCLAW":
      $cards = is_array($lastResult) ? $lastResult : explode(",", $lastResult);
      $totalReveal = 0;
      foreach ($cards as $card) {
        if (CardNameContains($card, "Crouching Tiger", $player)) ++$totalReveal;
      }
    ...
    case "REMOVECOUNTER":
      ...
      WriteLog(CardLink($parameter) . " removed a counter from " . CardLink($character[$lastResult]));
      return $lastResult;
    case "FINALIZEDAMAGE":
      $params = explode(",", $parameter);
      $damage = $dqVars[0];
    ...
    case "LORDSUTCLIFFE":
      ...
      return $lastResult;
    case "BINGO":
      if ($lastResult == "") WriteLog("No card was revealed for " . CardLink("bingo_red"));
      $cardType = CardType($lastResult);
      if ($cardType == "AA") {
        WriteLog(CardLink("bingo_red") . " gained go again");
        GiveAttackGoAgain();
      } else if (DelimStringContains($cardType, "A")) {
        WriteLog(CardLink("bingo_red") . " draw a card");
        Draw($player);
      } else WriteLog(CardLink("bingo_red") . "... did not hit the mark");
      return $lastResult;
    ...
    case "SETABILITYTYPE":
      ...
      if (!isset($names[$index])) {
        WriteLog("An error occurred with " . CardLink($parameter) . ". Please submit a bug report.");
        return "";
      }
      else {
        ...
        if (!str_contains($mzIndex[0], "ALLY")) {
          if ($mzIndex[1] == 0) {
            ...
            if ($damage > 0) {
              ...
              $targetCard = GetMZCard($player, $mzItem);
              WriteLog(CardLink($targetCard) . " was shot down from its perch!");
    ...
    case "SUCCUMBTOWINTER":
      ...
      switch ($params[0]) {
        case "THEIRALLY":
          $allies = &GetAllies($otherPlayer);
          WriteLog(CardLink($params[2]) . " destroyed your frozen ally");
        ...
        default:
          ...
          WriteLog(CardLink($params[2]) . " destroyed your frozen arsenal card");
          break;
      }
      return $lastResult;
    case "STARTGAME":
      ...
      foreach ($lastResultArr as $mzItem) {
        ...
        switch ($mzIndex[0]) {
          case "MYARS":
            ...
            WriteLog(CardLink($arsenal[$mzIndex[1]]) . " gained an aim counter.");
      ...
      foreach ($lastResultArr as $mzItem) {
        ...
        switch ($mzIndex[0]) {
          case "MYITEMS":
            $items = &GetItems($player);
            $items[$mzIndex[1] + 1] += 1;
            WriteLog(CardLink($items[$mzIndex[1]]) . " gained a steam counter.");
            break;
          case "MYARS":
            $arsenal = &GetArsenal($currentPlayer);
            $arsenal[$mzIndex[1] + 3] = 1;
            ...
            break;
          case "LAYER":
            AddCurrentTurnEffect($params[0]."-".GetMZCard($currentPlayer, $mzItem), $currentPlayer, "LAYER");
            WriteLog(GetMZCardLink($currentPlayer, $mzItem) . " gained a steam counter.");
            break;
          default:
            break;
        }
      ...
      foreach ($lastResultArr as $mzItem) {
        ...
        switch ($mzIndex[0]) {
          case "MYITEMS":
            $items = &GetItems($player);
            $items[$mzIndex[1] + 1] -= 1;
            WriteLog(CardLink($items[$mzIndex[1]]) . " lost a steam counter.");
      ...
      for ($i = count($lastResultArr) - 1; $i >= 0; --$i) {
        ...
        switch ($mzIndex[0]) {
          ...
          case "MYITEMS":
            ...
            $items[$mzIndex[1] + 1] = 0;
            WriteLog(CardLink($items[$mzIndex[1]]) . " lost all their steam counters");
          ...
          case "MYCHAR":
            ...
            WriteLog(CardLink($characters[$mzIndex[1]]) . " lost all their steam counters");
            break;
          default:
            $controller = $player;
            break;
    ...
    case "ONHITEFFECT":
      ...
      else WriteLog(CardLink("tripwire_trap_red") . " received an eratta that causes to prevent flick effects.");
    ...
    case "AWAKEN":
      ...
      WriteLog(CardLink($cardID) . " awakened into " . CardLink($newCardID));
    ...
    case "REMOVECOUNTERAURAORDESTROY":
      ...
      if ($lastResult == "YES") {
        --$auras[$index + 2];
        WriteLog("Player " . $playerID . " removed a counter from " . CardLink($auras[$index]) . ".");
      } else {
        WriteLog("Player " . $playerID . " did not remove a counter and " . CardLink($auras[$index]) . " was destroyed");
    ...
    case "REMOVECOUNTERITEMORDESTROY":
      ...
      if ($lastResult == "YES") --$items[$parameter + 1];
        ...
        WriteLog(CardLink($items[$parameter]) . " was destroyed");
    ...
    case "ADDBOTTOMREMOVETOP":
      ...
      WriteLog("⬇️ Player " . $player . " put " . CardLink($card) . " on the bottom of the deck and Clash again!");
      return "";
    case "CLASH":
    ...
    case "ALREADYDEAD":
      ...
      WriteLog(CardLink($lastResult) . " was banished.");
      return $lastResult;
    case "POWDERKEG":
      $index = FindCharacterIndex($defPlayer, $lastResult);
      DestroyCharacter($defPlayer, $index);
      WriteLog(CardLink($lastResult) . " was destroyed");
    ...
    case "ADDTRIGGER":
      ...
      if ($targetClass != "") {
        WriteLog(GetMZCardLink($targetedPlayer, $lastResult) . " targeted by " . CardLink($params[0]) . "'s trigger");
      }
    ...
    case "UNDERCURRENTDESIRES":
      if ($lastResult == "") {
        WriteLog("No cards were selected, " . CardLink("sacred_art_undercurrent_desires_blue") . " did not banish any cards");
        return $lastResult;
      }
      $cards = explode(",", $lastResult);
      $message = CardLink("sacred_art_undercurrent_desires_blue") . " banished ";
      $cardsCount = count($cards);
      for ($i = 0; $i < $cardsCount; ++$i) {
        if ($i >= 1) $message .= ", ";
        if ($i != 0 && $i == $cardsCount - 1) $message .= "and ";
    ...
    case "AMULETOFOBLATION":
      ...
      WriteLog(CardLink("amulet_of_oblation_blue") . " targeted " . CardLink($target));
      return $lastResult;
    case "FABRICATE":
    ...
    case "CURRENTATTACKBECOMES":
      WriteLog(CardLink($combatChain[0]) . " copy and become " . CardLink($lastResult));
    ...
    case "PROVOKE":
      ...
      else WriteLog(CardLink($cardID) . " could not be added as a blocking card");
      return $lastResult;
    case "COMPARENUMBERS":
      $otherPlayer = $player == 1 ? 2 : 1;
    ...
    case "GOLDENSKYWARDEN":
      if (GetMZCard($player, $lastResult) == "golden_cog") {
        ...
        WriteLog("Player " . $player . " created a " . CardLink("gold") . " token");
        return $lastResult;
      }
      else {
    ...
    case "REMOVEFROMCHOICES":
      ...
      foreach($choices as $choice) {
        if ($choice != $lastResult) $ret[] = $choice;
        else $lastResult = "USEDUP";
      }
      return implode(",", $ret);
    case "DEDUPE":
      return implode(",", array_unique(explode(",", $lastResult)));
    case "PITCHCARD":
      if (CanPitch($lastResult, $parameter)) {
        ...
        WriteLog("Pitching " . CardLink($lastResult) . " prevented");
      }
      return $lastResult;
