...
{
  global $CS_DamageTaken, $CS_ArcaneDamageTaken;
  $otherPlayer = $player == 1 ? 2 : 1;
  ...
  for ($i = count($otherAuras) - AuraPieces(); $i >= 0; $i -= AuraPieces()) {
    switch ($otherAuras[$i]) {
      case "ROS077":
        if (GetClassState($otherPlayer, $CS_DamageTaken) <= 0 && GetClassState($otherPlayer, $CS_ArcaneDamageTaken) <= 0 && $damage > 0 && $otherAuras[$i + 5] > 0) {
          $otherAuras[$i + 5] -= 1;
