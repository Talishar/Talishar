...
class bait extends Card {
  ...
  function PlayAbility($from, $resourcesPaid, $target = '-', $additionalCosts = '-', $uniqueID = '-1', $layerIndex = -1) {
    global $CombatChain;
    if (IsReactionPhase()) {
      if ($CombatChain->AttackCard()->ID() == $this->cardID) AddCurrentTurnEffect("$this->cardID-BUFF", $this->controller);
      else AddCurrentTurnEffectFromCombat("$this->cardID-BUFF", $this->controller);
    }
    return "";
  }
...
class battlefield_beacon_yellow extends Card {
  ...
  function ProcessAttackTrigger($target, $uniqueID) {
    ...
    AddDecisionQueue("SETDQCONTEXT", $this->controller, "Choose up to $count " . ($count == 1 ? "mode" : "modes"));
