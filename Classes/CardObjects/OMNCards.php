...
class twoscilio extends BaseCard
{
    ...
    function SpecificLogic()
    {
        ...
        if (TypeContains($Card->ID(), "I", $this->controller)) {
            AddCurrentTurnEffect(
                $this->cardID,
                $this->controller,
                uniqueID: $discardedUID,
            );
        }
    }
    ...
    function IsPlayRestricted($index)
    {
        ...
        if ($Auras->FindCardID("lightning_flow")->Index() == -1) {
            return true;
        }
        return false;
    }
...
class nebula_duality extends BaseCard
{
    ...
    function GetAbilityNames(
        ...
        return $this->archetype->GetAbilityNames(
            $index,
            $from,
            $foundNullTime,
            $layerCount,
...
class nebula_duality_red extends Card
{
    ...
    function GetAbilityNames(
        ...
        return $this->baseCard->GetAbilityNames(
            $index,
            $from,
            $foundNullTime,
            $layerCount,
...
class voltbound_duality_red extends Card
{
    ...
    {
        if (
            GetResolvedAbilityType($this->cardID, "HAND") == "I" &&
            $from == "HAND"
        ) {
            return 1;
        }
...
class zyggy_base extends BaseCard
{
    function IsPlayRestricted($index)
    {
        ...
        if (FindHoloAuras($this->controller) == "") {
            return true;
        }
        return false;
    }
