...
function ModalAbilities($player, $card, $lastResult, $index=-1)
{
  ...
  switch($card)
  {
    ...
    case "JUSTANICK":
      switch($lastResult) {
        ...
        case "Both":
          ...
          if (LinkBasePower() <= 1 && CardType($attackCardID) == "AA") AddCurrentTurnEffect("just_a_nick_red-BUFF", $player);
          if (HasStealth($attackCardID)) AddCurrentTurnEffect("just_a_nick_red-HIT", $player);
          break;
      }
      return $lastResult;
    case "MAUL":
    ...
    case "MICROPROCESSOR":
      ...
      $itemModsIndex = $index + 8;
      $modalities = explode(",", $items[$itemModsIndex]);
    ...
    case "SHIVER":
      ...
      $arsenalOffset = count($arsenal) - ArsenalPieces() + 5;
      switch($lastResult) {
        case "1_Attack":
          AddCurrentTurnEffect("shiver-1", $player, "HAND", $arsenal[$arsenalOffset]);
          return 1;
        case "Dominate":
          AddCurrentTurnEffect("shiver-2", $player, "HAND", $arsenal[$arsenalOffset]);
          return 1;
      }
      return $lastResult;
    case "VOLTAIRE":
      ...
      switch ($lastResult) {
        case "1_Attack":
          AddCurrentTurnEffect("voltaire_strike_twice-1", $player, "HAND", $arsenal[$arsenalOffset]);
          return 1;
        case "Go_again":
          AddCurrentTurnEffect("voltaire_strike_twice-2", $player, "HAND", $arsenal[$arsenalOffset]);
          return 1;
      }
      return $lastResult;
    case "KORSHEM":
  ...
  switch($params[0])
  {
    ...
    case "vile_inquisition_red": case "vile_inquisition_yellow": case "vile_inquisition_blue":
      if($params[0] == "vile_inquisition_red") $pitchTarget = 1;
        ...
        $pitchTarget = ($params[0] == "vile_inquisition_yellow") ? 2 : 3;
      $deck = new Deck($target);
      $banished = $deck->BanishTop();
      if (PitchValue($banished) == $pitchTarget)
        LoseHealth(1, $target);
      return "";
...
function SpecificCardLogic($player, $card, $lastResult, $initiator)
{
  ...
  switch ($params[0]) {
    ...
    case "ENCASEDAMAGE":
      ...
      for ($i = $charPieces; $i < $charCount; $i += $charPieces) {
        if (CardType($character[$i]) == "E" && $character[$i + 1] != 0)
          $character[$i + 8] = 1;//Freeze their equipment
      }
      return $lastResult;
    case "BLESSINGOFFOCUS":
      ...
      if ($deck->Reveal() && CardSubType($deck->Top()) == "Arrow") {
        if (!ArsenalFull($player)) {
          AddArsenal($deck->Top(true), $player, "DECK", "UP", 1);
        }
        else
          WriteLog("Your arsenal is full");
      }
      return $lastResult;
    ...
    case "SANDSKETCH":
      ...
      if (ModifiedPowerValue($discarded, $player, "HAND", source: "sand_sketched_plan_blue") >= 6)
    ...
    case "REMEMBRANCE":
      ...
      if (is_array($lastResult)) {
        ...
        WriteLog("Something unexpected happened with Remembrance, please submit a bug report");
      return "1";
    ...
    case "SOULREAPING":
      ...
      $countCards = count($cards);
      if ($countCards > 0)
        AddCurrentTurnEffect("soul_reaping_red", $player);
      $numBloodDebt = 0;
      for ($i = 0; $i < $countCards; ++$i)
    ...
    case "DIMENXXIONALGATEWAY":
      if (ClassContains($lastResult, "RUNEBLADE", $player))
        DealArcane(1, 0, "PLAYCARD", $EffectContext, true, resolvedTarget: "THIERCHAR-0");
    ...
    case "GENESIS":
      ...
      if (TalentContains($lastResult, "LIGHT", $player))
        Draw($player, false);
      if (ClassContains($lastResult, "ILLUSIONIST", $player))
    ...
    case "SPOILEDSKULL":
      ...
      if (substr_count($cleanIndexes, ',') + 1 < 3) {
    ...
    case "SYSTEMRESET":
      ...
      for ($i = count($lastResult) - 1; $i >= 0; --$i) {
        ...
        $destroyedItems[] = $cardID;
      }
      $countDestroyed = count($destroyedItems);
      for ($i = 0; $i < $countDestroyed; ++$i) {
    ...
    case "HYPERSCRAPPER":
      ...
      for ($i = $scrappedAmount - 1; $i >= 0; $i--) {
        ...
        if (CardName($cardID) == "Hyper Driver")
          $scrappedHyperDriverAmount++;
      }
    ...
    case "IRONFIST":
      ...
      ++$arsenal[$index + 6];
      return "";
    case "BATTLEFIELDBEACON":
      $modes = (!is_array($lastResult)) ? ($lastResult == "PASS" ? [] : explode(",", $lastResult)) : $lastResult;
