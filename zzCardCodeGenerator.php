<?php
  ...
  function GenerateCardObjectClassMap(): void
  {
    $cardObjectFiles = [
      'DBGCards.php', 'LGSCards.php', 'WTRCards.php', 'ARCCards.php',
      'MONCards.php', 'ELECards.php', 'EVRCards.php', 'UPRCards.php',
      'DYNCards.php', 'OUTCards.php', 'DTDCards.php', 'EVOCards.php',
      'HVYCards.php', 'MSTCards.php', 'ROSCards.php', 'AJVCards.php',
      'HNTCards.php', 'SEACards.php', 'AGBCards.php', 'SUPCards.php',
      'APSCards.php', 'ARRCards.php', 'PENCards.php', 'OMNCards.php',
      'AACCards.php', 'AHACards.php', 'AZSCards.php', 'MPWCards.php',
      'AOLCards.php', 'IARCards.php', 'AMACards.php', 'MPACards.php',
      'AMOCards.php',
    ];

    $sourceDirectory = __DIR__ . '/Classes/CardObjects/';
    $classMap = [];

    foreach ($cardObjectFiles as $fileName) {
      $source = file_get_contents($sourceDirectory . $fileName);
      if ($source === false) {
        throw new RuntimeException("Unable to read card object file: {$fileName}");
      }
      ...
      for ($i = 0; $i < $tokenCount; ++$i) {
        ...
        $className = strtolower($tokens[$i][1]);
        if (isset($classMap[$className]) && $classMap[$className] !== $fileName) {
          throw new RuntimeException(
            "Duplicate card class {$className} in {$classMap[$className]} and {$fileName}"
          );
        }
    ...
    $destination = __DIR__ . '/GeneratedCode/CardObjectClassMap.php';
    if (file_put_contents($destination, $output, LOCK_EX) === false) {
      throw new RuntimeException("Unable to write {$destination}");
    }

    echo 'Generated ', count($classMap), " card class mappings.<BR>";
  }
