...
try {
  ...
  $userIdSql = "SELECT usersId, isBanned FROM users WHERE usersUid = ?";
  ...
  if ($row = mysqli_fetch_assoc($result_ids)) {
    ...
    $userIsBanned = intval($row['isBanned']) == 1;
  }
