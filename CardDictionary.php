...
function CardCost($cardID, $from="-")
{
  ...
  switch($cardID)
  {
    ...
    case "HVY186": case "HVY187": case "HVY188":
      WriteLog($from);
      if(GetResolvedAbilityType($cardID, "HAND") == "AA" || $from == "CC" || $from == "PITCH") return 3;
      else return 0;
    case "HVY209":
      if(GetResolvedAbilityType($cardID, "HAND") == "AA" || $from == "CC" || $from == "PITCH") return 2;
      else return 0;
    default: break;
  }
  if($set != "ROG" && $set != "DUM") {
...
function PlayableFromBanish($cardID, $mod="", $nonLimitedOnly=false)
{
  ...
  $mod = explode("-", $mod)[0];
  if($mod == "INT" || $mod == "DTD564") return false;
