    ...
    if(count($decisionQueue) == 0 && count($layers) > 0) {
      ...
      if($priorityHeld) {
        ...
        if($cardID == "ENDTURN") EndStep();
          ...
          PlayCardEffect($cardID, $params[0], $params[1], $target, $additionalCosts, isset($params[3]) ? $params[3] : "-1", isset($params[2]) ? $params[2] : -1);          if(RequiresDieRoll($cardID, $params[0], $player)) ClearDieRoll($player);
        }
      }
...
function ProcessTrigger($player, $parameter, $uniqueID, $target="-", $additionalCosts="-", $from="-")
{
  ...
  switch($parameter) {
    ...
    case "CRU097":
      ...
      AddDecisionQueue("CHANGESHIYANABASELIFE", $player, "<-", 1);
