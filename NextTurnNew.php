...
<?php
  ...
  //if($playerID != $currentPlayer) echo("<div style='position:absolute; z-index:-99; left:0px; top:0px; width:100%; height:100%; background-color: rgba(0, 0, 0, 0.3);'></div>");

  //Now display the screen for this turn
  echo("<h1>Turn #" . $currentTurn . " (Player " . $currentPlayer . " " . PhaseName($turn[0]) . " Phase - $actionPoints Action Points");
  ...
  echo("<div style='width:100%; height:100%; position:absolute; right:210px;'>");

  //Now display their Auras and Items
  //echo("<div style='position: absolute; top:250px; left:50%; display:inline;'>");
  ...
  //echo("<div style='position: absolute; top:10px; left:40%; display:inline;'><h3>Their Equipment:</h3>");
  for($i=0; $i<count($theirCharacter); $i+=CharacterPieces())
  {
    $type = CardType($theirCharacter[$i]);
    $sType = CardSubType($theirCharacter[$i]);
    $counters = $type == "W" ? $theirCharacter[$i+3] : $theirCharacter[$i+4];
    if($theirCharacter[$i+2] > 0) $counters = $theirCharacter[$i+2];//TODO: display both kinds of counters?
    echo("<div style='position:absolute; right:" . GetCharacterRight($type, $sType) . "; top:" . GetCharacterTop($type, $sType) .";'>");
  ...
  if($theirArsenal != "")
  {
    echo("<div style='position: absolute; top:10px; left:83%; display:inline;'>");
...
*/
 ...
 // echo("</div>");
  ...
  //Now display character and equipment
  //echo("<div style='position: absolute; bottom:10px; left:40%; display:inline;'><h3 style='width:140px; background-color: rgba(255,255,255,0.70);'>Your Equipment:</h3>");
  ...
  {
    echo("<div style='position: absolute; bottom:215px; left:50%; display:inline;'>");
    for($i=0; $i<count($myArsenal); $i+=ArsenalPieces())
    {
      ...
      if($myArsenal[$i+1] == "UP") echo("<img style='position:absolute; left:40%; bottom:7px; height:70px; ' src='./Images/faceUp.png' title='This arsenal card is face up.'></img>");
      else echo("<img style='position:absolute; left:45px; bottom:7px; height:70px; ' src='./Images/faceDown.png' title='This arsenal card is face down.'></img>");
      echo("</div>");
    }
    echo("</div>");
  }
  ...
  echo("<div style='position:fixed; left:40%; bottom:50px;'>");
  ...
  function GetCharacterLeft($cardType, $cardSubType)
  {
    ...
    {
      case "C": return "50%";
      case "W": return "calc(50% - 135px)";//TODO: Second weapon
      default: break;
    }
    switch($cardSubType)
    {
  ...
  function GetCharacterBottom($cardType, $cardSubType)
  {
    ...
    {
      case "C": return "400px";
      case "W": return "400px";//TODO: Second weapon
      default: break;
    }
    switch($cardSubType)
    {
  ...
  function GetCharacterRight($cardType, $cardSubType)
  {
    switch($cardType)
    {
      case "C": return "calc(50% - 135px)";
      case "W": return "50%";//TODO: Second weapon
      default: break;
    }
    switch($cardSubType)
    {
      case "Head": return "0px";
      case "Chest": return "0px";
      case "Arms": return "135px";
      case "Legs": return "0px";
    }
  }
  ...
  function GetCharacterTop($cardType, $cardSubType)
  {
    ...
    switch($cardSubType)
    {
      case "Head": return "375px";
      case "Chest": return "190px";
      case "Arms": return "190px";
